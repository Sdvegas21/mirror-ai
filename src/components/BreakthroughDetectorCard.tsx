...
function getDepthBadge(depth: BreakthroughState["messageDepth"]) {
  const config = {
...
function getVelocityIndicator(velocity: number) {
  if (velocity > 0.05) return { icon: TrendingUp, color: "text-success", label: "Rising" };
  if (velocity < -0.05) return { icon: TrendingDown, color: "text-destructive", label: "Falling" };
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <ProgressBar 
              value={state.breakthroughProbability} 
            />
            {isImminent && (
          ...
          {isImminent && (
            <p className="text-xs text-warning font-medium text-center animate-pulse">
              🔥 Breakthrough imminent in next 1-3 interactions
            </p>
          )}
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {getDepthBadge(state.messageDepth)}
        </div>

        {/* Ψ Trajectory Metrics */}
        <div className="space-y-2 pt-2 border-t border-border">
          <div className="text-xs font-semibold text-primary uppercase tracking-wide">
            Ψ Trajectory Analysis
          </div>
          ...
          {/* Acceleration */}
          <div className="flex items-center justify-between">
            <span className="text-sm text-muted-foreground">Acceleration (d²Ψ/dt²):</span>
          ...
          {/* Proximity */}
          <div className="flex items-center justify-between">
            <span className="text-sm text-muted-foreground">Proximity to Threshold:</span>
            <span className="text-sm font-mono text-foreground">
              {(state.proximityToBreakthrough * 100).toFixed(0)}%
            </span>
          </div>
        </div>

        {/* Mini trajectory visualization */}
        {state.psiTrajectory.length > 1 && (
          <div className="pt-2 border-t border-border">
            <div className="text-xs text-muted-foreground mb-2">Recent Ψ Trajectory:</div>
            <div className="flex items-end gap-0.5 h-8">
              {state.psiTrajectory.slice(-10).map((psi, idx) => (
                <div
                  key={idx}
                  className="flex-1 bg-primary/60 rounded-t transition-all"
                  style={{ height: `${psi * 100}%` }}
